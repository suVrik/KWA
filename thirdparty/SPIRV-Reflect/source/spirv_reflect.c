...
#define calloc(number, size) p_allocator->calloc(p_allocator->context, (number), (size))

#ifdef free
#undef free
#endif

#define free(pointer) p_allocator->free(p_allocator->context, (pointer))
...
)
{
  ...
  p_parser->p_allocator = p_allocator;
...
{
  SpvAllocator* p_allocator = p_parser->p_allocator;
...
{
  assert(IsNotNull(p_parser));
  assert(IsNotNull(p_parser->p_allocator));
  assert(IsNotNull(p_parser->spirv_code));
...
static SpvReflectResult ParseEntryPoints(Parser* p_parser, SpvReflectShaderModule* p_module)
{
  ...
  if ((result = EnumerateAllUniforms(p_module, &uniform_count, &uniforms, p_allocator)) !=
       SPV_REFLECT_RESULT_SUCCESS) {
    return result;
  }
  size_t push_constant_count = 0;
  uint32_t* push_constants = NULL;
  if ((result = EnumerateAllPushConstants(p_module, &push_constant_count, &push_constants, p_allocator)) !=
       SPV_REFLECT_RESULT_SUCCESS) {
    return result;
  }
...
static SpvReflectResult ParseDescriptorSets(
  SpvReflectShaderModule* p_module,
  SpvAllocator*           p_allocator
)
{
  ...
  return ParseEntrypointDescriptorSets(p_module, p_allocator);
}
...
)
{
  ...
  SpvReflectResult result = ParseDescriptorSets(p_module, p_allocator);
  return result;
}
...
{
  return spvReflectCreateShaderModule(size, p_code, p_module, p_allocator);
}
...
)
{
  ...
  if (result == SPV_REFLECT_RESULT_SUCCESS) {
    result = SynchronizeDescriptorSets(p_module, p_allocator);
    SPV_REFLECT_ASSERT(result == SPV_REFLECT_RESULT_SUCCESS);
  }
  if (result == SPV_REFLECT_RESULT_SUCCESS) {
  ...
  if (result != SPV_REFLECT_RESULT_SUCCESS) {
    spvReflectDestroyShaderModule(p_module, p_allocator);
  }
...
static void SafeFreeBlockVariables(
  SpvReflectBlockVariable* p_block,
  SpvAllocator*            p_allocator
)
{
  if (IsNull(p_block)) {
...
void spvReflectDestroyShaderModule(
  SpvReflectShaderModule* p_module,
  SpvAllocator*           p_allocator
)
{
  ...
  for (size_t i = 0; i < p_module->entry_point_count; ++i) {
    ...
    for (size_t j = 0; j < p_entry->interface_variable_count; j++) {
      SafeFreeInterfaceVariable(&p_entry->interface_variables[j], p_allocator);
    }
    for (uint32_t j = 0; j < p_entry->descriptor_set_count; ++j) {
      SafeFree(p_entry->descriptor_sets[j].bindings);
    }
...
const SpvReflectDescriptorBinding* spvReflectGetDescriptorBindingByName(
    const SpvReflectShaderModule* p_module,
    const char* name,
    SpvReflectResult* p_result
)
{
  ...
  if (IsNotNull(p_module)) {
    for (uint32_t index = 0; index < p_module->descriptor_binding_count; ++index) {
      const SpvReflectDescriptorBinding* p_potential = &p_module->descriptor_bindings[index];
      if ((p_potential->name != NULL) && (strcmp(p_potential->name, name) == 0)) {
        p_descriptor = p_potential;
        break;
      }
    }
