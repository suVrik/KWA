...
constexpr bool equal(const plane& lhs, const plane& rhs, float epsilon = EPSILON) {
    return equal(lhs.normal, rhs.normal, epsilon) && equal(lhs.distance, rhs.distance, epsilon);
}

inline bool isfinite(const plane& value) {
