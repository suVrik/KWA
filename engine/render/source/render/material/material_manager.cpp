...
size_t MaterialManager::GraphicsPipelineHash::operator()(const std::pair<String, String>& value) const {
    return std::hash<String>()(value.first) ^ std::hash<String>()(value.second);
}
...
public:
    GraphicsPipelineTask(MaterialManager& manager, GraphicsPipelineContext& graphics_pipeline_context, const String& vertex_shader, const String& fragment_shader)
        : m_manager(manager)
    ...
    void run() override {
        ...
        instance_attribute_descriptors[0].offset = offsetof(Material::InstanceData, model) + offsetof(float4x4, _r0);
        ...
        instance_attribute_descriptors[1].offset = offsetof(Material::InstanceData, model) + offsetof(float4x4, _r1);
        ...
        instance_attribute_descriptors[2].offset = offsetof(Material::InstanceData, model) + offsetof(float4x4, _r2);
        ...
        instance_attribute_descriptors[3].offset = offsetof(Material::InstanceData, model) + offsetof(float4x4, _r3);
        ...
        instance_binding_descriptor.stride = sizeof(Material::InstanceData);
        ...
        uniform_sampler_descriptor.anisotropy_enable = true;
        ...
        graphics_pipeline_descriptor.render_pass_name = "geometry_render_pass";
        ...
        graphics_pipeline_descriptor.is_stencil_test_enabled = true;
...
public:
    ...
    void run() override {
        ...
        SharedPtr<GraphicsPipeline*> graphics_pipeline_context = m_manager.load(
            vertex_shader.get_value().c_str(), fragment_shader.get_value().c_str(),
            texture_names, is_skinned.get_value(),
            m_graphics_pipeline_end
        );
        ...
        m_material = Material(graphics_pipeline_context, std::move(material_textures), is_skinned.get_value());
    }
...
public:
    ...
    void run() override {
        ...
        for (auto& [relative_path, material] : m_manager.m_pending_materials) {
            MaterialTask* material_task = new (m_manager.m_transient_memory_resource.allocate<MaterialTask>()) MaterialTask(m_manager, *material, relative_path, m_graphics_pipeline_end_task);
...
MaterialManagerTasks MaterialManager::create_tasks() {
    Task* material_end_task = new (m_transient_memory_resource.allocate<NoopTask>()) NoopTask("Material Manager Material End");
    Task* graphics_pipeline_end_task = new (m_transient_memory_resource.allocate<NoopTask>()) NoopTask("Material Manager Graphics Pipeline End");
    Task* begin_task = new (m_transient_memory_resource.allocate<BeginTask>()) BeginTask(*this, material_end_task, graphics_pipeline_end_task);

    return { begin_task, material_end_task, graphics_pipeline_end_task };
}

SharedPtr<GraphicsPipeline*> MaterialManager::load(const char* vertex_shader, const char* fragment_shader,
                                                   ...
                                                   Task* graphics_pipeline_end) {
    {
        ...
        auto it = m_graphics_pipelines.find(std::pair<String, String>{
...

    {
        ...
        auto [it, success] = m_graphics_pipelines.emplace(std::pair<String, String>{
        ...
        GraphicsPipelineTask* material_task = new (m_transient_memory_resource.allocate<GraphicsPipelineTask>()) GraphicsPipelineTask(*this, it->second, it->first.first, it->first.second);
