...
ParticleSystemDescriptor ParticleSystemReflection::create_from_markdown(const ParticleSystemReflectionDescriptor& descriptor) {
    ...
    result.duration = node["duration"].as<NumberNode>().get_value();
    ...
    result.max_bounds = MarkdownUtils::aabbox_from_markdown(node["max_bounds"]);
    result.shadow_material = descriptor.material_manager->load(node["shadow_material"].as<StringNode>().get_value().c_str());
