...
public:
    ...
    void run() override {
        ...
        Vector<LightPrimitive*> primitives = m_manager.m_scene.query_lights(m_manager.m_camera_manager.get_occlusion_camera().get_frustum());

        std::sort(primitives.begin(), primitives.end(), LightSort(m_manager.m_camera_manager.get_occlusion_camera()));
        
        Vector<PointLightPrimitive*> shadow_lights(m_manager.m_transient_memory_resource);
    ...
    , m_transient_memory_resource(*descriptor.transient_memory_resource)
{
    ...
    for (ShadowMap& shadow_map : m_shadow_maps) {
        ...
        if (!descriptor.disable_translucent_shadows) {
...
Texture* ShadowManager::get_color_texture(LightPrimitive* light_primitive) const {
    for (const ShadowMap& shadow_map : m_shadow_maps) {
        if (shadow_map.light_primitive == light_primitive) {
            // Translucent shadow maps could be disabled.
            return shadow_map.color_texture ? shadow_map.color_texture : m_dummy_color_texture;
        }
    }
