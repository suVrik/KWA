...
class VelocityOverLifetimeParticleSystemUpdater : public OverLifetimeParticleSystemUpdater<float3> {
public:
    static ParticleSystemUpdater* create_from_markdown(MemoryResource& memory_resource, ObjectNode& node);

    VelocityOverLifetimeParticleSystemUpdater(Vector<float>&& inputs, Vector<float3>&& outputs);
