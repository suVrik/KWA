...
Texture2D tonemapping_uniform_attachment;
...
[[vk::push_constant]] FxaaPushConstants fxaa_push_constants;

float4 main(FS_INPUT input) : SV_TARGET {
    FxaaTex fxaa_texture;
    fxaa_texture.smpl = sampler_uniform;
    fxaa_texture.tex = tonemapping_uniform_attachment;

    return float4(FxaaPixelShader(input.texcoord, fxaa_texture, fxaa_push_constants.texel_size.xy), 1.0);
}
